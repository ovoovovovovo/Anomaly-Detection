...
def reset_state():
    return gr.update(value=''), None, None, [], [], [], PILImage.open('back.png')


with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.HTML("""<h1 align="center" color = 'red'>Anomaly Detection</h1>""")

    with gr.Row():
        with gr.Column(scale=1):
            with gr.Row(scale=3):
                image_path = gr.Image(type="filepath", label="待检测图像", value=None)
            with gr.Row(scale=3):
                normal_img_path = gr.Image(type="filepath", label="正常样本", value=None)
            ...
            with gr.Row():
                with gr.Column(scale=6):
                    chatbot = gr.Chatbot().style(
                        height=475,
                        border_radius=8,  # 添加圆角
                        container=False  # 去除默认容器边距
                    )
                with gr.Column(scale=4):
                    image_output = gr.Image(
                        ...
                        shape=[350, 350],
                        type='pil',
                        value=PILImage.open('back.png')
                    )
                    # —— 在热力图下方新增一组滑块，与 Chatbot 同高 —— #
                    with gr.Column():
                        max_length = gr.Slider(
                            minimum=0, maximum=512, value=512, step=1.0,
                            label="Maximum length", interactive=True
                        )
                        top_p = gr.Slider(
                            minimum=0, maximum=1, value=0.01, step=0.01,
                            label="Output Diversity（Top P）", interactive=True
                        )
            with gr.Row():
                user_input = gr.Textbox(show_label=False, placeholder="请输入...", lines=5).style(container=False)
            with gr.Row():
